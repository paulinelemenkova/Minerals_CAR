...
\begin{figure}[H]
	...
	\caption{Geologic provinces in CAR. Data source: USGS. {Background topographic map: OSGeo}.
...
\begin{figure}[H]
	...
	\label{fig03}}  %MDPI: ``0,000'' seems not correct for latitude and longitude. please check and revise. % <-- Authors' reply: yes, Figure 3 is redone and corrected. 0,000 is changed to 0 throughout in the cartographic grid. Unnecessary zeros are removed.
\end{figure}

The southern region of CAR lithology continues along %Please check intended meaning has been retained % <-- Authors' reply: yes, correct (retained).
...
%----------- subsection ----------->
\subsection{Geoid}
The first processing step consisted of converting the ADF file into the GDR by `grdconvert' using the tile to the extent of CAR applied to the area of `n00e00' in the global gravity data (Listing \ref{lst02}). Afterwards, the~extent of the data range was evaluated for the image {by} the `gdalinfo'{ }and the {visualization} was adjusted{ by} `makecpt'{. }Extracting the image was done using the `grdimage' module with the extensions applied to the actual extent of the country (-R14/28/2/11.5), %MDPI: Please check if this should be a minus sign ("$-$" U+2212), if so, please revise it. % <-- Authors' reply: yes, it should be a minus sign: it is a syntax of the programming code GMT where a minus sign signifies the start of the command. It is correct.
...
In this section, we present the results of this study which we briefly summarize as follows: (1) %MDPI: Open parenthesis have been added. Please confirm. % <-- Authors' reply: yes, we confirm here and in the next similar changes until nr. 5.
...
\centering
	...
	\end{adjustwidth}
%}
	\caption{Histogram equalisation of the topography grids on CAR based on the ETOPO1 raster data. \textbf{(a)} original grid; \textbf{(b)} equalised grid; \textbf{(c)} normalised grid; \textbf{(d)} quadratic grid. Data source: ETOPO1. %MDPI: please label these sub-figures with a,b,c,d and give an explanation for them. % <-- Authors' reply: corrected (added labels a,b,c,d in sub-figures).
	\label{fig04}}
\end{figure}
%\vspace{-9pt}

The analysis of the topographic grid reveals a local depression in the equalised raster grid which corresponds to the geoid and gravity data in the central region of CAR. Furthermore, the~data are contrasted in the equalised and quadratic models (Figure \ref{fig04}b,d) %MDPI: There are not notes for subfigures within Figure 4 (a,b,c,d). Please check and revise (add notes within the figure) if necessary. % <-- Authors' reply: notes are added in the subfigures within Figure 4. 
compared to the original and normalised grids (Figure \ref{fig04}a,c), where topographic derivatives reveal a saddle-shaped structure in the 1 min %MDPI: Please check if space can be added insted of hyphen (-). % <-- Authors' reply: yes, agree (added space instead of hyphen).
...
The gravity anomalies are caused by the rugged surface, and non-linear and complex composition of the Earth's inner structure. %Please check intended meaning has been retained. % <-- Authors' reply: yes, retained.
Other factors include thinner lithospheric thickness and Moho depths associated with the isostatic surface topography~\cite{Stunff} which affects its gravitational field. Furthermore, the~values of gravity are affected by the rotation of the Earth which results in values near the equator being higher compared to those on the poles. %Please check intended meaning has been retained. % <-- Authors' reply: yes, retained.
Beneath the magnetic anomaly, the~geoid in CAR has values ranging from $-$4 to $-$8 m, which is lower compared to that of the western region where values above 16 m were recorded. %Please check intended meaning has been retained. % <-- Authors' reply: yes, retained.
...
\begin{figure}[H]
	...
	% <-- Authors' reply: yes, corrected: the hyphen is changed into a minus sign in Figure 5 for ``T= -18/18''.
\end{figure}
%\vspace{-9pt}
...
\begin{figure}[H]
	...
	% <-- Authors' reply: yes, corrected: the hyphen is changed into a minus sign in Figure 6.
\end{figure}
...
\begin{figure}[H]
	...
	% <-- Authors' reply: yes, corrected: the hyphen is changed into a minus sign in Figure 7 for ``T= -40/40'''.
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Magnetic anomaly in CAR based on WDMAM with 3-arc-minute resolution. Dark blue areas indicate the negative Bangui magnetic anomaly in the centre of CAR. Data source: WDMAM~\cite{Lesur}.
	\label{fig08}}%MDPI: Please confirm if need change the hyphen (-) into a minus sign ($-$, "U+2212") for ``-T-933/933''.
	% <-- Authors' reply: yes, corrected: the hyphen is changed into a minus sign in Figure 8 for ``T= -933/933'''.
\end{figure}
