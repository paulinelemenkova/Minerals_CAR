...
\hl{Each cartographic matching method supports either a specific data format, e.g., like shape files -- .shp in the ArcGIS} \cite{LAR2023e01436,NGENE2022360,CHENG2022106052} \hl{or the tiled format for image processing in remote sensing software} \cite{KONWEA2023e01597,ALKHERSAN2022104678} \hl{or a limited set of converted and imported data formats from the multi-source data} \cite{SAHA2022104706,APEH2022104561,SHEBL2022765}. \hl{Scripting and programming methods also showed their effectiveness in matching tasks and coherence analysis when dealing with topographic and geophysical datasets since their optimise the workflow by smooth, automated and rapid approaches in data processing. For instance, scripts facilitate modelling the geochemical-geophysical inversion to investigate the issues of dynamic topography} \cite{Jones}, \hl{enable automated and optimised isolines approximation and mesh gradation in topographic data processing} \cite{GORMAN20081721}, \hl{or support detailed topographic analysis through 3D cross-sections} \cite{Sobh,Lemenkova202301}.

\hl{In this paper, we address the problem of the unified cartographic matching framework that supports a large family of data formats technically implemented in the Generic Mapping Tools (GMT) and QGIS. Our goal is to present a }scripting cartographic approach\hl{ }that exploits the structural regularities in the geophysical, geologic and topographic datasets on \hl{Central African Republic }(CAR) and uses them as input information during\hl{ }matching procedure. \hl{With this regard, this study aims at presenting }a cartographic interpretation of the Bangui Magnetic Anomaly and \hl{analysis of} its consistence \hl{using} the available data on geophysical and geological setting of the CAR. Technically, we optimised the operational workflow of the cartographic work due to the use of scripts. 

\hl{To this end, }\hl{we introduced the }scripting cartographic framework that \hl{utilises the} codes written \hl{by the }GMT syntax for automation of \hl{the }cartographic workflow. We first extract all the information from the gravity, topography and magnetic grids and map their parameters\hl{. Afterwards, we detect and separately analyse }the repeated structures in the topographic grids. This is done using equalisation and comparison of the equalised, quadratic, normalized and original rasters through enhanced  ranges of a grid depending on frequency distribution of elevation amplitude of the relief in various regions of CAR. The \hl{raster }images were incorporated independently \hl{and processed }as separate files\hl{. The scripts used for implementation of the cartographic work} are presented in the Appendix section with added comments on the most essential details of the GMT codes in Methodology\hl{.}
...
Revealed by the satellite magnetometer data and reported in previous studies \cite{Regan,OUABEGO201311,NJITEUTCHOUKEU2021104206}, the anomaly has a crustal origin and is strongly related to the tectonic evolution and Precambrian history of the Saharan metacraton and Congo Craton. Geologically, it is located in the region of \hl{the }deposited highly metamorphosed Precambrian rocks and has a Precambrian basement with large masses of \hl{the} magmatic rocks \cite{Kochemasov}. The formation of the anomaly is related to the lithosphere bending. \cite{NJITEUTCHOUKEU2021104299} reports the thick crust and \hl{a }strong lithosphere beneath the anomaly, as well as high coherence between the Bouguer gravity and relief which correlates with the Moho depths near \hl{the }anomaly.

At the same time, \hl{the comparison between the magnetic contrasts, geologic setting and geophysical data in the Earth's critical zones provides new} insights into the formation of the continental margins and distribution of mineral resources. \hl{It furthermore allows a better interpretation of the effects of the past tectonic activity on the present geophysical processes} \cite{FELIXTOTEU2002906}. Thus, regions earlier affected by the geodynamic processes are notable for negative magnetic anomalies, which suggests a strong correlation between the active tectonics and the origin of the magnetic anomalies which is associated with the lithosphere movements \cite{RAVAT199259}. For instance, \cite{ARKANIHAMED1985257} noted that the lithosphere beneath the sedimentary basins formed during the Gondwana break up has a lower geomagnetism compared to the regions under the cratons, which is related to the thermal demagnetisation of the high-temperature asthenosphere. Further, certain effects are noted from the magnetic mineralogy of the ascending magma in the lower crust.

\hl{The} magmatism \hl{in Central Africa }is \hl{a consequence of }the continental rifting\hl{, opening of the Equatorial Atlantic and the Mesozoic-Cenozoic magmatic activity} \cite{WILSON1992203}. The lithospheric extension and \hl{the }associated ascent of magma resulted in \hl{the }intrusion of \hl{the }tholeitic basalts \hl{which have} typical geochemical and isotopic signatures. The tectonic structure of \hl{ }CAR is notable for the \hl{distribution} of the Central African Fold Belt \hl{which extends in the } north-east \hl{direction from }Cameroon through CAR towards Sudan and contributed to the tectonic magmatism. The Central African Fold Belt is formed as a result of the extension of the Congo Craton and associated collisions\hl{ }of the related crustal blocks in Cameroon and CAR \cite{TOTEU2022296}. The Mesozoic separation of African crustal blocks during the Early Cretaceous resulted in \hl{the }formation of the Western Central African Rift System. As a result, the \hl{crude oil deposits are formed }in \hl{the }structured reservoirs of \hl{the }fault blocks \hl{in} the rift basins \hl{of} CAR \cite{GENIK1992169}.

\hl{The }anomalies in the lithospheric magnetic field present the data on \hl{the }variation in the intensity of Earth's magnetisation which points at both explicit and hidden geophysical properties. In turn, data on magnetisation reveal\hl{ }the properties of \hl{the }underlying rock, the tectonic history of Earth and help interpreting the key steps in the geologic evolution of the Earth \cite{Vervelidou}. At the same time, our understanding of these processes and the complexity of their interactions using Earth observation data can assist in practical decision making on natural resource management, sustainable environmental development and mineral exploration in CAR. 
